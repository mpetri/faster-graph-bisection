...
pub fn recursive_graph_bisection(
    ...
    rayon::scope(|s| {
        ...
        s.spawn(|_| {
            recursive_graph_bisection(&mut left, num_terms, iterations, min_partition_size, progress_left);
        });
        s.spawn(|_| {
            recursive_graph_bisection(&mut right, num_terms, iterations, min_partition_size, progress_right);
        });
    });
}